  ...
  useEffect(() => {
    ;(window as any).cameraControlFunctions = {
      fitToView: () => {
        ...
        // Set rotation center AND focal point to bounding box center
        useAppStore.getState().setRotationCenter([center.x, center.y, center.z])
        if (controlsRef.current?.lookAt) {
          controlsRef.current.lookAt(center)
        } else {
      ...
      snapToPlane: (plane: 'xy' | 'yz' | 'xz') => {
        // Get rotation center from store
        const rc = useAppStore.getState().cameraSettings.rotationCenter
        const target = new THREE.Vector3(rc[0], rc[1], rc[2])
        ...
        // Reset focal point to rotation center for a clean axis-aligned view
        if (controlsRef.current?.lookAt) {
      ...
      rotateCamera: (degrees: number) => {
        // Rotate the up vector around the viewing direction (roll)
        ...
        // Re-orient toward current focal point
        if (controlsRef.current?.focalPoint) {
          camera.lookAt(controlsRef.current.focalPoint)
        }
